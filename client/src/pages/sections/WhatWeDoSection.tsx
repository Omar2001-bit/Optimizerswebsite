    ...
    <section className="w-full bg-dark-mode900 py-[100px] px-8">
      ...
      <div className="max-w-[1376px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-[16px]">
        {serviceCards.slice(0, 4).map((card, index) => (
          ...
          >
            <CardContent
              className="flex flex-col items-center justify-between gap-4 p-6 relative w-full h-full min-h-[280px] rounded-2xl border border-solid border-[#6ae49933] backdrop-blur-[7.5px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(7.5px)_brightness(100%)]"
              style={{
            ...
            >
              ...
              <img
                className="absolute top-0 right-[57px] w-[180px] h-2 opacity-30"
                alt="Mask group"
                src={card.topMask}
              />

              <img
                className="absolute left-[57px] bottom-[-7px] w-[180px] h-2 opacity-30"
                alt="Mask group"
                src={card.bottomMask}
              />
            </CardContent>
